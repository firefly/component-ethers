/**
 *  This is modified slightly to use the Firefly hash utilities and
 *  match the rest of the API. ~Richard Moore <me@ricmoo.com>.
 *
 *  See original author license below.
 */
...
static inline void sha256_extract(FfxSha256Context *restrict ctx, uint8_t *restrict out)
{
...
static inline void sha256_cpy(FfxSha256Context *restrict out, const FfxSha256Context *restrict in)
{
  out->h[0] = in->h[0];
  out->h[1] = in->h[1];
  out->h[2] = in->h[2];
...
static inline void sha256_xor(FfxSha256Context *restrict out, const FfxSha256Context *restrict in)
{
  out->h[0] ^= in->h[0];
  out->h[1] ^= in->h[1];
  out->h[2] ^= in->h[2];
...
static void sha256_final(uint8_t *out, FfxSha256Context *ctx) {
    ffx_hash_finalSha256(ctx, out);
}

DECL_PBKDF2(sha256,
            _ffx_sha256_block_length,
            FFX_SHA256_DIGEST_LENGTH,
            FfxSha256Context,
            ffx_hash_initSha256,
            ffx_hash_updateSha256,
            _ffx_hash_xformSha256,
            sha256_final,
            sha256_cpy,
            sha256_extract,
            sha256_xor)

static inline void sha512_extract(FfxSha512Context *restrict ctx, uint8_t *restrict out)
{
...
static inline void sha512_cpy(FfxSha512Context *restrict out, const FfxSha512Context *restrict in)
{
  out->h[0] = in->h[0];
  out->h[1] = in->h[1];
  out->h[2] = in->h[2];
...
static inline void sha512_xor(FfxSha512Context *restrict out, const FfxSha512Context *restrict in)
{
  out->h[0] ^= in->h[0];
  out->h[1] ^= in->h[1];
  out->h[2] ^= in->h[2];
...
static void sha512_final(uint8_t *out, FfxSha512Context *ctx) {
    ffx_hash_finalSha512(ctx, out);
}

DECL_PBKDF2(sha512,
            _ffx_sha512_block_length,
            FFX_SHA512_DIGEST_LENGTH,
            FfxSha512Context,
            ffx_hash_initSha512,
            ffx_hash_updateSha512,
            _ffx_hash_xformSha512,
            sha512_final,
            sha512_cpy,
            sha512_extract,
            sha512_xor)
...
void ffx_pbkdf2_sha256(uint8_t *dkOut, size_t dkLength, uint32_t iterations,
  ...
  const uint8_t *salt, size_t saltLength) {
    PBKDF2(sha256)(passwd, passwdLength, salt, saltLength, iterations,
      dkOut, dkLength);
}
...
void ffx_pbkdf2_sha512(uint8_t *dkOut, size_t dkLength, uint32_t iterations,
  const uint8_t *passwd, size_t passwdLength,
  const uint8_t *salt, size_t saltLength) {
    PBKDF2(sha512)(passwd, passwdLength, salt, saltLength, iterations,
      dkOut, dkLength);
}
